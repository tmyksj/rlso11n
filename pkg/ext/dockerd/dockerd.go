...
func Start() error {
	...
	if err := writeRlsDScript(scriptPath); err != nil {
		return errors.By(err, "failed to create script file")
	}
	...
	if out, err := rlsD.StdoutPipe(); err != nil {
		return errors.By(err, "failed to pipe stdout")
	} else {
		go pipeStdout(out)
	}

	if out, err := rlsD.StderrPipe(); err != nil {
		return errors.By(err, "failed to pipe stderr")
	} else {
		go pipeStderr(out)
	}

	if err := rlsD.Start(); err != nil {
		return errors.By(err, "failed to start dockerd")
	}

	core.RegisterFinalizer(rlsDFinalize)
	if err := util.WaitListenUnix(context.DockerSock()); err != nil {
...
if [ -z $_DOCKERD_ROOTLESS_CHILD ]; then
	...
	[ $_DOCKERD_ROOTLESS_CHILD = 1 ]
	echo "`+rlsDPidPrefix+`$$"
	rm -f /run/docker /run/xtables.lock /var/lib/docker
	rootlessctl --socket=$ROOTLESSKIT_STATE_DIR/api.sock add-ports `+context.MyAddr()+`:2377:2377/tcp
	rootlessctl --socket=$ROOTLESSKIT_STATE_DIR/api.sock add-ports `+context.MyAddr()+`:7946:7946/tcp
	rootlessctl --socket=$ROOTLESSKIT_STATE_DIR/api.sock add-ports `+context.MyAddr()+`:7946:7946/udp
	rootlessctl --socket=$ROOTLESSKIT_STATE_DIR/api.sock add-ports `+context.MyAddr()+`:4789:4789/udp
